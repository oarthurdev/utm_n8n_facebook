    ...
    <Card className="bg-white dark:bg-gray-800 shadow rounded-lg overflow-hidden mb-8">
      ...
      <CardContent className="p-6">
        {isLoading ? (
          ...
          <>
            <div className="mb-4">
              <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">API Credentials Status</p>
                ...
                {credentials.map((credential) => (
                  ...
                  >
                    <span className={cn(
                      ...
                      credential.status === 'set' ? 'text-success' : 'text-error'
                    )}>
                      {credential.status === 'set' ? 'check_circle' : 'error'}
                    </span>
                    <span className="text-sm text-gray-700 dark:text-gray-300">{credential.key}</span>
                    <span className={cn(
                      "text-xs ml-auto",
                      credential.status === 'set' ? 'text-gray-500 dark:text-gray-400' : 'text-error'
                    )}>
            ...
            <div className="mt-6">
              <div className="flex items-center justify-between">
                <p className="text-sm font-medium text-gray-700 dark:text-gray-300">Environment Actions</p>
              </div>
              <div className="mt-3 flex flex-wrap gap-3">
                ...
                >
                  <span className="material-icons text-gray-500 dark:text-gray-400 mr-2 text-sm">restart_alt</span>
                ...
                <Button 
                  ...
                  className="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary"
                >
                  <span className="material-icons text-gray-500 dark:text-gray-400 mr-2 text-sm">build</span>
                  Test Connections
                </Button>
                <Button 
                  variant="outline" 
                  className="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary"
                >
                  <span className="material-icons text-gray-500 dark:text-gray-400 mr-2 text-sm">sync</span>
                ...
                >
                  <span className="material-icons text-error mr-2 text-sm">settings</span>
                  Manage API Keys
                </Button>
              </div>
            </div>
